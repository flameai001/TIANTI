...
function sortEventsForManager(value: Event[]) {
  return [...value].sort((left, right) => Date.parse(left.startsAt) - Date.parse(right.startsAt));
}
...
function createArchiveEntry(talentId: string, sceneAssetId: string): EditorArchive["entries"][number] {
  return {
    id: crypto.randomUUID(),
    talentId,
    sceneAssetId,
...
function validateEventDraft(
  eventDraft: EditableEvent,
  editableLineups: EditableLineup[],
  liveEvents: Event[]
) {
  if (!eventDraft.name.trim()) return "请先填写活动名称。";
  if (!eventDraft.startsAt) return "请先填写开始时间。";
  if (!eventDraft.city.trim()) return "请先填写城市。";
  if (!eventDraft.venue.trim()) return "请先填写场馆。";
  if (!eventDraft.note.trim()) return "请先填写活动备注。";

  const duplicateSlug = liveEvents.find(
    (event) => event.slug === eventDraft.slug.trim() && event.id !== eventDraft.id
  );
  if (eventDraft.slug.trim() && duplicateSlug) {
    return "该活动 slug 已存在，请修改后再保存。";
  }

  for (const lineup of editableLineups) {
    if (!lineup.talentId) return "阵容里还有达人未选择。";
    if (!lineup.source.trim()) return "请为每位达人填写信息来源。";
    if (!lineup.note.trim()) return "请为每位达人填写补充备注。";
  }
...
export function ArchiveManager({
  ...
  function resetDrafts(
    ...
    setEventDraft(createEventDraft(nextEvents.find((event) => event.id === eventId) ?? null));
    setEditableLineups(createLineupDrafts(eventId, nextLineups));
    setArchiveDraft(createArchiveDraft(eventId, nextArchives));
    setMessage(null);
  ...
  function handleNewEvent() {
    if (!canLeaveCurrentWork()) return;
    resetDrafts(null, liveEvents, liveLineups, liveArchives);
  ...
  async function handleSaveEvent() {
    const validationError = validateEventDraft(eventDraft, editableLineups, liveEvents);
    if (validationError) {
    ...
    startTransition(async () => {
      ...
      const nextLineups = [
        ...
        ...editableLineups.map((lineup) => ({ ...lineup, eventId: nextEventId }))
      ];
  ...
  async function handleDeleteEvent() {
    ...
    if (!window.confirm(`确定删除 ${selectedEvent.name} 吗？`)) return;
  ...
  async function handleBulkEventAction(action: "set_status" | "delete") {
    ...
    if (action === "delete" && !window.confirm(`确定批量删除 ${selectedEventIds.length} 个活动吗？`)) {
      return;
    }
            ...
            <section className="surface rounded-[1.8rem] p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-3 text-sm leading-7 text-white/60">
                    支持从当前阵容一键导入，再按每条记录微调，避免重复录入同场活动信息。
                  </p>
                </div>
