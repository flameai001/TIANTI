...
export function TalentManager({ talents, assets }: TalentManagerProps) {
  ...
  const coverAssets = assets.filter((asset) => asset.kind === "talent_cover");
  const representationAssets = assets.filter((asset) => asset.kind === "talent_representation");
        ...
        <form action={handleSave} className="space-y-5">
          ...
          <div className="flex items-center justify-between gap-4">
            <p className="text-xs leading-6 text-white/45">
              当前是演示流程，图片先使用站内演示素材 ID；接入 R2 后可切到真实上传。
            </p>
            <button
              disabled={pending}
